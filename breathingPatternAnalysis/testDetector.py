...
def plot(df: pd.DataFrame,
    ...
    # ── Top panel: original signals + detected peaks ──────────────────────────
    ax1.set_facecolor("#f9f9f9")
    ax1.plot(t, df["Raw_Value"],      color="#378ADD", lw=0.9, alpha=0.75, label="Raw ADC")
    ax1.plot(t, df["Filtered_Value"], color="#E9A020", lw=2.0,             label="Filtered EMA")
    ax1.plot(t, baseline,             color="#888888", lw=1.2, linestyle="--",
             alpha=0.7, label="Baseline (drift)")

    ax1.scatter(t.iloc[peaks], df["Filtered_Value"].iloc[peaks],
                color="#E24B4A", edgecolors="#A32D2D",
                s=90, zorder=5, linewidths=1.5, label="Exhale detected")
    ...
    stats_lines = [
        ...
        f"Recording length : {stats['total_sec']:.1f} s",
    ]
    ax1.text(0.01, 0.97, "\n".join(stats_lines),
    ...
    ax1.set_title("Breathing Pattern - Exhale Detection & BPM", fontsize=13,
                  fontweight="bold", pad=10)
    ax1.legend(loc="lower right", fontsize=9, framealpha=0.9)
    ax1.grid(True, color="#cccccc", linewidth=0.5, linestyle="--", alpha=0.7)

    # ── Bottom panel: detrended signal (what the detector actually sees) ──────
    ax2.set_facecolor("#f9f9f9")
    ax2.plot(t, detrended, color="#7B68EE", lw=1.2, label="Detrended EMA")
    ax2.axhline(0, color="#999", lw=0.8, linestyle="--")

    ax2.scatter(t.iloc[peaks], detrended[peaks],
                color="#E24B4A", edgecolors="#A32D2D",
                s=70, zorder=5, linewidths=1.3)

    ax2.set_ylabel("Detrended ADC", fontsize=10)
    ax2.set_xlabel("Time", fontsize=11)
    ax2.legend(loc="lower right", fontsize=9, framealpha=0.9)
    ax2.grid(True, color="#cccccc", linewidth=0.5, linestyle="--", alpha=0.7)
