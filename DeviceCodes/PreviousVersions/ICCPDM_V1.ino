...
//---------------------------------------------------------- Sensor Veriables
float filteredValue = 2000.0;

// ---------------------------- Auto Calibration
int sensorBaseline  = 0;      // idle value captured at startup
int sensorRange     = 1000;    // expected working range above baseline — tune this
bool isCalibrated   = false;

//-------------------------------------------------------------- Air Preasure Sensor
float temperature = 28.3; //Air
...
void loop()
{
  ...
  // -------------------------------------------------------- Sensor Value Read
  int sensorValue = readSensor();   // replaces analogRead(sensorPin)
  ...
  if (systemMode == MODE_AUTO)
  {
    ...
    if (systemMode == MODE_AUTO)
    {
      int BLDC_Speed = map(sensorValue, sensorBaseline, sensorBaseline + sensorRange, 1074, 2000);
      targetSpeed = constrain(BLDC_Speed, 1074, 2000);
      SpeedLVL    = map(targetSpeed, 1074, 2000, 0, 10);
      rampToSpeed(targetSpeed);
    } 
    // ********************** CALIBRATION *************
    //int BLDC_Speed = map(sensorValue, 300, 3100, 1074, 2000);
    // targetSpeed = constrain(BLDC_Speed, 1074, 2000);
  ...
  if      (systemMode == MODE_AUTO)   modeLabel = "AUTO";
  else if (systemMode == MODE_MANUAL) modeLabel = "MANUAL";
  else                                modeLabel = "NONE";
...
int readSensor() 
{
    ...
    //---------------------------------------- 2 EMA to smooth the median
    const float alpha = 0.2;
    filteredValue = (alpha * median) + ((1.0 - alpha) * filteredValue);
...
void calibrateSensor() {
    SerialBT.println("Calibrating sensor...");
    SerialBT.println("Keep device in RESTING state for 3 seconds.");
    ...
    for (int i = 0; i < CAL_SAMPLES; i++) 
    {
        ...
        delay(60);   // spread over ~3 seconds total
    }

    sensorBaseline = sum / CAL_SAMPLES;
    isCalibrated   = true;

    Serial.print("Baseline captured: ");
    Serial.println(sensorBaseline);

    SerialBT.print("Calibration done! Baseline: ");
    SerialBT.println(sensorBaseline);
...
void parseBTInput()
{
  ...
  {
    systemMode = MODE_NONE;      // pause motor logic during calibration
    targetSpeed = 1074;
    rampToSpeed(targetSpeed);
    calibrateSensor();
    sendModePrompt();            // ask user to pick mode again after cal
    return;
  }
